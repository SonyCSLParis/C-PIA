...
def launcher(train, load, overfitted, config):
    # === Init process group
    ...
    mp.spawn(
        main,
        args=(train, load, overfitted, config, world_size, model_dir),
        nprocs=world_size,
        join=True,
    )


def main(rank, train, load, overfitted, config, world_size, model_dir):
