...
class FastAttention_(nn.Module):
    ...
    def forward(self, q, k, q_rot, k_rot, v, states, inferring_states):
        ...
        if states is not None:
            assert (
                q.size(2) == 1
            ), "recurrent inference can only be applied to sequences of len 1"
            ...
            if inferring_states:
                ...
                out = causal_linear_attention(
                    q, k, q_rot, k_rot, v, local=self.window_size
                )
                states = None
        return out, states
...
def causal_linear_attention(q, k, q_rot, k_rot, v, local=None, eps=1e-6):
    ...
    if q_rot is None:
        ...
        if local is not None:
            N_shifted = get_N(q_local, k_local, v_local) + get_N(
                q_rot_local, k_rot_local, v_local
            )
            N[:, :, local:] = N[:, :, local:] - N_shifted
...
def infer_hidden_states(q, k, q_rot, k_rot, v, chunk_size=128, eps=1e-6):
    ...
    for q, k, q_rot, k_rot, v in zip(
        *map(lambda t: t.chunk(num_chunks, dim=-2), (q, k, q_rot, k_rot, v))
    ):
        ...
        if q_rot is not None:
            for q_rot, k_rot in zip(
                *map(lambda t: t.chunk(num_chunks, dim=-2), (q_rot, k_rot))
            ):
                k_cumsum_rot = last_k_cumsum_rot + k_rot.cumsum(dim=-2)
                D_rot = torch.einsum(
                    "...nd,...nd->...n", q_rot, k_cumsum_rot.type_as(q_rot)
                )
                context_rot = torch.einsum("...nd,...ne->...nde", k_rot, v)
                context_cumsum_rot = last_context_cumsum_rot + context_rot.cumsum(
                    dim=-3
                )
            D_inv = 1.0 / (D + D_rot + eps)
        else:
            D_inv = 1.0 / (D + eps)

        out = torch.einsum("...nde,...nd,...n->...ne", context_cumsum, q, D_inv)
        if q_rot is not None:
            out_rot = torch.einsum(
                "...nde,...nd,...n->...ne", context_cumsum_rot, q_rot, D_inv
            )
            out = out + out_rot
...
def recursive_attention_step(q, k, q_rot, k_rot, v, states, eps=1e-12):
    ...
    D = torch.einsum("...nd,...nd->...n", q, k_cumsum.type_as(q))
    if q_rot is not None:
        D_rot = torch.einsum("...nd,...nd->...n", q_rot, k_cumsum_rot.type_as(q_rot))
        D_inv = 1.0 / (D + D_rot + eps)
    else:
        D_inv = 1.0 / (D + eps)
