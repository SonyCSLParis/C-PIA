...
class Performer_(nn.Module):
    ...
    def forward(self, x, pos_emb_input, **kwargs):
        ...
        assert (
            n <= self.max_seq_len
        ), f"sequence length {n} must be less than \
...
class _Performer_(nn.Module):
    def __init__(
        ...
        ), "tuple specifying number of local attention heads per depth \
        ...
        attn_route_map = {
            "mask": route_attn,
            "pos_emb_input": route_attn,
            "inferring_states": route_attn,
            "states": route_attn,
        }
        context_route_map = (
